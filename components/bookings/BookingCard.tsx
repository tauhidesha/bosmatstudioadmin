          ...
          <div className="rounded-xl border border-slate-200 p-4 bg-slate-50">
            {(booking.downPayment || 0) > 0 && (
              <div className="flex justify-between text-xs text-slate-500 mb-2">
                <span>Subtotal</span>
                <span>Rp {(booking.totalAmount || booking.subtotal || 0).toLocaleString()}</span>
              </div>
            )}
            {(booking.downPayment || 0) > 0 && (
              <div className="flex justify-between text-xs text-amber-600 mb-2">
                <span>DP (sudah dibayar)</span>
                <span>- Rp {(booking.downPayment || 0).toLocaleString()}</span>
              </div>
            )}
            <p className="text-xs font-bold text-slate-500 mb-1">{(booking.downPayment || 0) > 0 ? 'Sisa Tagihan:' : 'Total Tagihan:'}</p>
            <p className="text-xl font-black text-slate-800">
              Rp { Math.max(0, (booking.totalAmount || booking.subtotal || 0) - (booking.downPayment || 0)).toLocaleString() || '0' }
            </p>
          </div>
