...
export default function BookingsClient({ initialBookings }: { initialBookings?: Booking[] }) {
  const { bookings, loading, updateBookingStatus, deleteBooking, updateBooking, addBookingLocally } = useBookings({ initialData: initialBookings });
  ...
  const handlePayAndInvoice = async () => {
    ...
    try {
      ...
      updateBookingStatus(paymentModal.id, 'paid');
    } catch (err: any) {
